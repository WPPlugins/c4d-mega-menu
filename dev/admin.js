	...
	$(document).ready(function(){
		$('#menu-to-edit > .menu-item').each(function(index, el){
			if ($(el).hasClass('menu-item-depth-0') || $(el).hasClass('menu-item-depth-1')) {
				$(el).find('.item-title').append('<span title="Enable Widget" class="c4d-mega-menu-button"><i class="fa fa-gear"></i></span>');	
			...
			if ($(el).hasClass('menu-item-depth-1')) {
				var current = $(el).find('.edit-menu-item-classes').val(),
				col = current.match(/col-sm-[0-9]*/);
				col = (col && col.length) ? col[0].replace('col-sm-', '').trim() : 12;
		...
		$('.c4d-mega-menu-button-col span').on('click', function(){
			...
			if ($(this).hasClass('prev')) {
				if (current > 1) {
					...
					input.val((input.val().replace('col-sm-' + current, '') + ' col-sm-' + (current - 1)).trim());
				}
